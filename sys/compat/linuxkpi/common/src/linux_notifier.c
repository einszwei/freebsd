...
#if defined(__arm__)
	if (total_scan > 0)
		new_nr = atomic_fetchadd_64((volatile uint64_t *)&shrinker->nr_deferred[nid].counter, total_scan);
	else
		new_nr = atomic_load_acq_64((volatile uint64_t *)&shrinker->nr_deferred[nid].counter);
#else
	if (total_scan > 0)
